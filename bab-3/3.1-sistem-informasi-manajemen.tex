...
Sistem Informasi Manajemen terdiri atas beberapa komponen yang meliputi komponen input, komponen model, komponen output, komponen teknologi, komponen \emph{hardware}, komponen \emph{software}, komponen basis data, dan komponen kontrol \cite{rusdiana2014sim}. Seluruh komponen tersebut saling berinteraksi satu sama lain untuk mencapai tujuan sistem informasi manajemen. Penjelasan setiap komponen nya adalah sebagai berikut:
...
\begin{itemize}
    ...
    Teknologi merupakan "\emph{tool box}" dalam sistem informasi. Teknologi digunakan untuk menerima input, menjalankan model, menyimpan dan mengakses data, menghasilkan dan mengirimkan keluaran, dan membantu pengendalian dari sistem secara keseluruhan.
    \item Komponen \emph{Hardware} \\
    \emph{Hardware} berperan penting sebagai suatu media penyimpanan vital bagi sistem informasi yang berfungsi sebagai \emph{database} sumber data dan informasi untuk memperlancar dan mempermudah kerja dari sistem informasi.
    \item Komponen \emph{Software} \\
    \emph{Software} berfungsi sebagai tempat untuk mengolah, menghitung, dan memanipulasi data yang diambil dari \emph{hardware} untuk menciptakan suatu informasi.
    \item Komponen Basis Data \\
    Basis data (\emph{database}) merupakan kumpulan data yang saling berkaitan dan berhubungan satu dengan yang lain, tersimpan di perangkat keras komputer dan menggunakan perangkat lunak untuk Sistem Informasi Manajemen memanipulasinya. Data perlu disimpan dalam basis data untuk keperluan penyediaan informasi lebih lanjut. Data dalam basis data perlu diorganisasikan sedemikian rupa informasi yang dihasilkan berkualitas. Organisasi basis data yang baik juga berguna untuk efisiensi kapasitas penyimpanannya. Basis data diakses atau dimanipulasi menggunakan perangkat lunak yang disebut\emph{ Database Management System} (DBMS).
